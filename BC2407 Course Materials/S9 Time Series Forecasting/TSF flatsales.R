...
# Learning Activity 2
# Train-test split at end 2018 -------------------------------------------
trainset <- ts(flatsales.ts[-c(49:52)], frequency = 4, start = c(2007,1))

testset <- ts(flatsales.ts[c(49:52)], frequency = 4, start = c(2019,1))


# Winter's Method on Train-Test -------------------------------------------
m.winters.2018 <- HoltWinters(trainset, seasonal = "multiplicative")

m.winters.2018

m.winters.2018.forecasts <- forecast(m.winters.2018, h = 4)

m.winters.2018.forecasts$mean        # Forecasts for 2019 4 periods.

m.winters.err <- testset - m.winters.2018.forecasts$mean

RMSE.winters.testerr <- round(sqrt(mean(m.winters.err^2)))
## 102

# Get list of error stats
accuracy(m.winters.2018.forecasts, testset)
