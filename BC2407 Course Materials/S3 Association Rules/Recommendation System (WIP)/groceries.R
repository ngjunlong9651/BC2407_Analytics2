...
# Ans: Smaller --> Rule Support, Coverage --> Antecendent Support
...
rules.lhs.yogurt.df <- as(rules.lhs.yogurt, "dataframe")

inspect(subset(rules, subset = lhs %in% c("yogurt", "rice")))
inspect(subset(rules, subset = lhs %ain% c("yogurt", "rice")))
inspect(subset(rules, subset = lhs %oin% c("yogurt", "rice")))
inspect(subset(rules, subset = lhs %pin% c("yogurt", "rice")))

# Q4: Get the Support, Conf and Lift for 2 specific rules:
#       {curd,yogurt} => {whole milk} and {rolls/buns} => {butter}

two.rules <- rules (lhs = list(c("curd", "yogurt"), c ("rolls/buns")),
                    rhs = list(c("whole milk"), c("butter")),
                    itemLabels = itemLabels(Groceries))
quality(two.rules) <- interestMeasure(two.rules,
